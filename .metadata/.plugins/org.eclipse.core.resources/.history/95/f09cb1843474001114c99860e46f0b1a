...
public class GameUIAdapter implements GameListener {
    ...
    private final PlayerController controller; // Usamos el contrato genérico inyectado
    ...
    public GameUIAdapter(PlayerController controller, GameConfig config, Boolean fixedPerspectiveP1, SwingUI vistaGrafica) {
        ...
        this.controller = controller; // Guardamos la referencia local del controlador activo
        ...
        // 3. Vinculamos la barra de texto de Swing con el loop de comandos
    ...
    private synchronized void ejecutarAccionYActualizarUI(Runnable accionEventual) {
        try {
            // Redirigimos temporalmente la salida de System.out a nuestro buffer
            System.setOut(printStreamAlternativo);
            
            // Ejecutamos el override original de tu ConsoleUI (puede ser onShotFired, onError, etc.)
            accionEventual.run();
            System.out.flush();
            ...
            if (textoGenerado.contains("=========================================")) {
                // ¡MAGIA AQUÍ! Si es un cambio de estado con tablero, LIMPIAMOS la pantalla
                vistaGrafica.limpiarPantalla();
                // E imprimimos el tablero fresco
                vistaGrafica.printLine(textoGenerado);
            } else {
                // Si NO es un tablero (es decir, es un onShotFired, onShipPlaced o onError)
                // NO limpiamos la pantalla. Lo agregamos al final para que se lea ABAJO del tablero.
                vistaGrafica.printLine(textoGenerado);
            }
            
        } finally {
            // Restauramos el System.out original
    ...
    private void procesarEntradaGrafica(String input) {
        ...
        // Simulamos la entrada de datos inyectando el comando directamente en el controlador
        try {
            ejecutarAccionYActualizarUI(() -> {
                ...
                // Usamos el controller inyectado aplicando polimorfismo puro de la interfaz
                if (parts.length >= 3) {
