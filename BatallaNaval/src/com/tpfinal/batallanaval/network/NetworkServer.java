            ...
            try (ServerSocket serverSocket = new ServerSocket(port)) {
                ...
                game.addListener(new GameListener() {
                    ...
                    public void onShipPlaced(Ship ship, int remaining) {
                        try {
                            synchronized (output) {
                                output.writeObject("PLACED:" + remaining);
                            }
                        } catch (Exception e) {}
                    }
                    ...
                    public void onShotFired(Position pos, ShotResult result, boolean p1Turn) {
                        try {
                            synchronized (output) {
                                ...
                                output.writeObject("SHOT_FIRED " + pos.getX() + " " + pos.getY() + " " + result.name() + " " + p1Turn);
                            }
                        } catch (Exception e) {}
                    }
                });
                ...
                while (true) {
                    ...
                    if (command.startsWith("PLACE")) {
                        ...
                        Direction dir = parts[3].equals("HORIZONTAL") ? Direction.HORIZONTAL : Direction.VERTICAL;
                    ...
                    else if (command.startsWith("SHOT")) {
                        String[] parts = command.split(" ");
                        int x = Integer.parseInt(parts[1]);
                        int y = Integer.parseInt(parts[2]);
